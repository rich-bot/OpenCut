...
import { resolveStickerAsset } from "@/stickers/resolver";
...
function isValidStickerId(value: unknown): value is string {
	...
	try {
		...
		if (!stickersRegistry.has(parsed.providerId)) {
			return false;
		}

		const asset = resolveStickerAsset({ stickerId: value });
		return asset.metadata?.missingSticker !== true;
	} catch {
		return false;
	}
}
